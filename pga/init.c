...
int init_run()
  ...
  /* calculate global variables */
   ...
   if (!strcmp(Base, "binary"))
      ...
      {
      sprintf(Xters, "abcdefghijklmnopqrstuvwxyz");
      Alphabet_size = 26;
      }
   else if (!strcmp(Base, "multichar"))
      {
      init_multichar();
      Alphabet_size = strlen(Xters);
      }
   else
      Alphabet_size = 0;
  ...
  /* May want to get rid of this restriction and just make it so that
   ...
   if (Variable_gen_len)
      ...
      if (Min_gen_len < Hx_window)
         ...
         printf(" not allowed\n");
         return ERROR;
         }  /* if min len < hx window */
      }  /* variable len */

  /* 010620AW:  make sure initial lengths (for init pop) are valid */
   if (Variable_gen_len == 1)
      {
      if (Init_min_gen_len > Init_max_gen_len)
         {
         printf(" Error(init_run): Init_min_gen_len(%d) > Init_max_gen_len(%d)",
			Init_min_gen_len, Init_max_gen_len);
         printf(" not allowed\n");
         return ERROR;
         }  /* if init min greater than init max */
      if (Init_min_gen_len < Min_gen_len)
         {
         Init_min_gen_len = Min_gen_len;
         printf(" Warning(init_run):");
         printf(" Init_min_gen_len too small, reset to Min_gen_len:%d\n",
                Min_gen_len);
         }  /* if init min len too small */
      if (Init_max_gen_len > Max_gen_len)
         {
         Init_max_gen_len = Max_gen_len;
         printf(" Warning(init_run):");
         printf(" Init_max_gen_len too large, reset to Max_gen_len:%d\n",
                Max_gen_len);
         }  /* if init max len too large */
      }  /* variable len == 1 */
...
int make_directory()
   ...
   sprintf(tempstring, "mkdir %s/run.%ld", Output_path, Run_num);
   ...
   if (Output_file[get_file_pointer("random")].on)
      {
      sprintf(tempstring, "%s/run.%ld/run.%ld.random",
...
/* parameters:
   called by:   init_run(), init.c
   actions:     Check to make sure there are no duplicated charaters
		in the multichar alphabet.  Duplicate characters are
		deleted.  Reset Xters to have no duplicates.
*/
void init_multichar()
   {
   int i, j, k;
...
#ifdef DEBUG
   printf(" ---in init_multichar---\n");
#endif

   len = strlen(Xters);

  /* compare each letter with rest of string for duplicates */
   for (i=0; i<len; i++)
      {
      j = i + 1;
      while (j<len)
