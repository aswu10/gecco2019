...
*/
   ...
   printf(" ---in ga_start()---\n");
#endif
   error = ga_init(params_file, opfiles_file, fxn_file, xover_rate, mut_rate, parsimony);
...
*/
int ga_init(char *params_file, char *opfiles_file, char *fxn_file, double xover_rate, double mut_rate, double parsimony)
   {
#ifdef DEBUG
   printf(" ---in ga_init()---\n");
#endif
  ...
  /* allocate space */
   ...
   Xters = (char *)malloc(INPUT_LINE_LEN * sizeof(char));
  ...
  /* read overriding parameter values and outputfiles */
   ...
   Mut_rate = mut_rate;
   Parsimony_pressure = parsimony;

  /* initialize run stuff */
   if (init_run() == ERROR)  return ERROR;
	/* include calculating global var in init_run? */

  /* initialize trace file stuff */
   if (Trace == 1)  if (init_trace() == ERROR)  return ERROR;

  /* initialize CoG directory if necessary */
   if (CoG == 1)  if (init_cog() == ERROR)  return ERROR;
  ...
  /* end of gen stats and calculations */
...
int ga_end()
  ...
  /* finish up any run stats */
   if (run_end() == ERROR)  return ERROR;

  /* finish up trace stuff */
   if (Trace == 1)  if (end_trace() == ERROR)  return ERROR;
