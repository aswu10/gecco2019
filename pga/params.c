...
int set_param(char *aline)
   ...
   else if (!strcmp(name, "Xters"))  sscanf(temp, "%s", Xters);
   ...
   else if (!strcmp(name, "Init_max_gen_len"))
		sscanf(temp, "%d", &Init_max_gen_len);
   else if (!strcmp(name, "Init_min_gen_len"))
		sscanf(temp, "%d", &Init_min_gen_len);
   ...
   else if (!strcmp(name, "Print_fxn_best"))
		sscanf(temp, "%d", &Print_fxn_best);
   else if (!strcmp(name, "Print_targets"))  sscanf(temp, "%d", &Print_targets);
   else if (!strcmp(name, "Trace"))  sscanf(temp, "%d", &Trace);
   else if (!strcmp(name, "CoG"))  sscanf(temp, "%d", &CoG);
   else
      {
  ...
  /* i starts at 1 because array element 0 should always be for
   ...
   for (i=1; i<Max_num_output_files; i++)
      ...
      if (Output_file[i].on)
         ...
         sprintf(Output_file[i].filename, "%s/run.%ld/run.%ld.%s",
