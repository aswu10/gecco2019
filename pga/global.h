...
char *Run_num_file;
char *Output_path;
char *Base;			/* nucleotides: binary or letter or multichar */
char *Xters;			/* if Base = multichar, specifies alphabet */
int Max_num_gen;
...
int Max_gen_len;		/* max gen len */
int Min_gen_len;		/* min gen len */
int Init_max_gen_len;		/* max gen len of init pop */
int Init_min_gen_len;		/* min gen len of init pop */
				/* defaults to Max/Min_gen_len if bad value */
...
int Print_targets;		/* if 1, print target changes */

				/* about saving trace data for entire run */
int Trace;			/* if 1, save all run data in */
				/* Output_path/trace directory in VIS format */

int CoG;			/* save centers of gravity for each xter */
				/* if 1, save one file of data for each *
				/* generation. */

int Xter_count;			/* record xter counts for full indv */
				/* and segments of indvs */
				/* if 1, print one file for each xter */
				/* into directory Output_path/xter_count */

/********** parameters read in from elsewhere **********/
long Run_num;			/* read in from Run_num_file */
...
RR rr;
SIM sim;
MATCH match;			/* 010426AW for fx.match function */
RA ra;				/* 010620AW for resource alloc fxn */
TENSOR tensor;
TSP tsp;
