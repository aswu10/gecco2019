...
int reproduce()
  ...
  /* mutate */
  mutate_pop();
   // if (!strcmp(Base, "binary"))  mutate_pop();
   // else if (!strcmp(Base, "alphabet"))  mutate_pop_alpha();
   // else if (!strcmp(Base, "multichar"))  mutate_pop_multichar();
#ifdef SMALLSTEP
...
void calc_num_offspring()
   ...
   if (!Sigma_scaling_on)
      ...
      for (i=Pop_size-1; i>=0; i--)
         Pop[i]->calc_num_offspring = Pop[i]->fitness/
			Prev_gen.fitness_sum*Pop_size;
      }  /* if */
   else
      {
      if (Prev_gen.std_dev < 0.0001)
         {
         for (i=Pop_size-1; i>=0; i--)
         ...
         for (i=Pop_size-1; i>=0; i--)
            ...
            Pop[i]->calc_num_offspring = 
                fmin(Sigma_scale_max,
                    fmax(Sigma_scale_min,
                         (1.0 + (Pop[i]->fitness - Prev_gen.avg_fitness) /
                              (2.0 * Prev_gen.std_dev) 
                         )
                        ));
            }
         }  /* else */
